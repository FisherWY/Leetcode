...
public class LCP08 {
    ...
    public int[] getTriggerTime(int[][] increase, int[][] requirements) {
        ...
        for (int i = 0; i < requirements.length; i++) {
            res[i] = fun(arr, requirements[i]);
        }
        return res;
    }

    // 二分查找
    private int fun(int[][] arr, int[] requirement) {
        int left = 0, right = arr.length - 1;
        while (left < right) {
            int mid = (left + right) / 2;
            if (arr[mid][0] >= requirement[0] && arr[mid][1] >= requirement[1] && arr[mid][2] >= requirement[2]) {
                right = mid;
            } else {
                left = mid + 1;
            }
        }
        if (arr[left][0] >= requirement[0] && arr[left][1] >= requirement[1] && arr[left][2] >= requirement[2]) {
            return left;
        } else {
            return -1;
        }
    ...
    public static void main(String[] args) {
        ...
        for (int x : lcp08.getTriggerTime(increase, requirements)) {
            System.out.print(x + " ");
        }
    }
}

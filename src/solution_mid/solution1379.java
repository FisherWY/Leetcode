...
import solution_hard.solution124;
...
public class solution1379 {
    ...
    private TreeNode find(final TreeNode node, final TreeNode target) {
        if (node == null) {
            return null;
        }
        ...
        TreeNode leftRes = find(node.left, target);
        TreeNode rightRes = find(node.right, target);
    ...
    public final TreeNode getTargetCopy1(final TreeNode original, final TreeNode cloned, final TreeNode target) {
        ...
        TreeNode leftRes = getTargetCopy1(original.left, cloned.left, target);
        if (leftRes != null) {
            return leftRes;
        }

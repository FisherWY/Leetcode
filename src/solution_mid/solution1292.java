...
public class solution1292 {
    ...
    public int maxSideLength(int[][] mat, int threshold) {
        ...
        for (int i = 1; i <= row; i++) {
            for (int j = 1; j <= col; j++) {
                for (int x = res + 1; x <= r; x++) {
                    if (i + x - 1 <= row && j + x - 1 <= col && getRect(prefix, i, j, i + x - 1, j + x - 1) <= threshold) {
                        res++;
                    } else {
                        break;
                    }
    ...
    private int getRect(int[][] prefix, int x1, int y1, int x2, int y2) {

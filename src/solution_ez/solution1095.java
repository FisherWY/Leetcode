...
public class solution1095 {
    ...
    public int findInMountainArray(int target, MountainArray mountainArr) {
        ...
        int res = findLeft(mountainArr, left, top, target);
        if (res != -1) {
            return res;
        }
        return findRight(mountainArr, top + 1, right, target);
    }
    ...
    private int findLeft(MountainArray mountainArray, int left, int right, int target) {
        while (left < right) {
            int mid = left + (right - left) / 2;
    ...
    private int findRight(MountainArray mountainArray, int left, int right, int target) {
        while (left < right) {
            int mid = left + (right - left) / 2;
            int value = mountainArray.get(mid);
            if (value == target) {
